...
#FTP 
iptables -A INPUT -p tcp -s $NET --sport 1024:65535 -d $IP --dport 21 -m state --state NEW,ESTABLISHED -j ACCEPT
iptables -A OUTPUT -p tcp -s $IP --sport 21 -d $NET --dport 1024:65535 -m state --state ESTABLISHED -j ACCEPT
iptables -A INPUT -p tcp -s $NET --sport 1024:65535 -d $IP --dport 1024:65535 -m state --state ESTABLISHED,RELATED -j ACCEPT
iptables -A OUTPUT -p tcp -s $IP --sport 1024:65535 -d 0/0 --dport 1024:65535 -m state --state ESTABLISHED -j ACCEPT
iptables -A OUTPUT -p tcp -s $IP --sport 20 -d $NET --dport 1024:65535 -m state --state ESTABLISHED,RELATED -j ACCEPT
iptables -A INPUT -p tcp -s $NET --sport 1024:65535 -d $IP --dport 20 -m state --state ESTABLISHED -j ACCEPT
...
$IPT -A INPUT -i $IF -p tcp --syn -s $NET --sport $HI_PORTS -d $IP --dport $port -m state --state NEW -m length --length 40:60 -m connlimit ! --connlimit-above 14 -j ACCEPT

#PACKET RAGE LIMIT
$IPT -N SYN_CHECK
$IPT -A SYN_CHECK -m recent --set --name SYN
$IPT -A INPUT -i $IF -p tcp --syn -s $NET --sport $HI_PORTS -d $IP --dport $port -m limit --limit 3/s --limit-burst 6 -m state --state NEW -m length --length 40:60 -j SYN_CHECK
$IPT -A SYN_CHECK -m recent --update --seconds 60 --hitcount 14  --name SYN -j ACCEPT
...
if test $port -eq 80
then
$IPT -t mangle -A PREROUTING -p tcp -s $NET --sport $HI_PORTS -d $IP --dport $port -m recent --hitcount 14 --name SYN --update --seconds 180 -j blockip
fi
done
